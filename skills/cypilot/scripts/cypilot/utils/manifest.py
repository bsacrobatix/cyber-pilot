...
def parse_manifest_v2(path: Path) -> ManifestV2:
    ...
    version = str(meta.get("version", "") or data.get("version", "")).strip()
    if not version:
...
def _parse_v1_as_v2(path: Path, data: Dict[str, Any]) -> ManifestV2:
...
def _parse_v2_sections(path: Path, data: Dict[str, Any]) -> ManifestV2:
    ...
    includes = list(meta.get("includes", []) or data.get("includes", []))
...
def _rewrite_component_paths(
    ...
    resolving them relative to *included_dir*.
    """
    ...
    for fname in ("prompt_file", "source"):
        ...
        if val and not Path(val).is_absolute():
            kwargs[fname] = str((included_dir / val).resolve())
        else:
            kwargs[fname] = val
...
def resolve_includes(
    ...
    for include_path_str in manifest.includes:
        ...
        rewritten_agents = [_rewrite_component_paths(c, included_dir) for c in included_manifest.agents]
        rewritten_skills = [_rewrite_component_paths(c, included_dir) for c in included_manifest.skills]
        rewritten_workflows = [_rewrite_component_paths(c, included_dir) for c in included_manifest.workflows]
        rewritten_rules = [_rewrite_component_paths(c, included_dir) for c in included_manifest.rules]
...
def apply_section_appends(
    ...
    """Compose content by appending section-append contributions from layers.

    Starts with *base_content* (the winning component's content), then for
    each loaded layer that declares ``append`` content for *component_id*,
    appends that content separated by a newline.  Layers are processed in
    resolution order (outermost first), so outer layer appends appear before
    inner layer appends.
    ...
    Args:
        ...
        layers:        List of ``ManifestLayer`` in resolution order.
    ...
    # @cpt-begin:cpt-cypilot-algo-project-extensibility-section-appending:p1:inst-step2-foreach-layer
    for layer in layers:
        if layer.state != ManifestLayerState.LOADED or layer.manifest is None:
            continue
        ...
        # @cpt-begin:cpt-cypilot-algo-project-extensibility-section-appending:p1:inst-step2.1-append
        for component in all_components:
            if component.id == component_id and component.append:
                composed = composed + "\n" + component.append
                break
        # @cpt-end:cpt-cypilot-algo-project-extensibility-section-appending:p1:inst-step2.1-append
    # @cpt-end:cpt-cypilot-algo-project-extensibility-section-appending:p1:inst-step2-foreach-layer
