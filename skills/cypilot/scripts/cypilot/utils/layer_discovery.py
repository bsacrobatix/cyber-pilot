...
def _detect_master_repo(start_dir: Path) -> Optional[Path]:
    ...
    while True:
        ...
        if canonical in visited:
            # Symlink cycle detected — stop walking
            return None
...
def _load_master_layer(master_root: Path) -> Optional[ManifestLayer]:
    ...
    try:
        ...
        rewritten_workflows = [_rewrite_component_paths(c, manifest_dir, trusted_root) for c in manifest.workflows]
        rewritten_rules = [_rewrite_component_paths(c, manifest_dir, trusted_root) for c in manifest.rules]
        manifest = dc_replace(manifest, agents=rewritten_agents, skills=rewritten_skills,
                              workflows=rewritten_workflows, rules=rewritten_rules)
        return ManifestLayer(
            scope="master",
            path=manifest_path,
            manifest=manifest,
