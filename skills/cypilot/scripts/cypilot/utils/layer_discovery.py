...
def _load_kit_layers(cypilot_root: Path) -> List[ManifestLayer]:
    ...
    for slug, kit_entry in kits_section.items():
        ...
        manifest_path = kit_path / "manifest.toml"
        if not manifest_path.is_file():
...
def _load_master_layer(master_root: Path) -> Optional[ManifestLayer]:
    ...
    Returns a ``ManifestLayer`` with ``PARSE_ERROR`` state if parsing fails.
    """
    manifest_path = master_root / "manifest.toml"
    if not manifest_path.is_file():
        return None
    ...
    try:
        ...
        rewritten_agents = [_rewrite_component_paths(c, manifest_dir) for c in manifest.agents]
        rewritten_skills = [_rewrite_component_paths(c, manifest_dir) for c in manifest.skills]
...
def discover_layers(repo_root: Path, cypilot_root: Path) -> List[ManifestLayer]:
    ...
    # Step 3-6: Walk up from repo_root parent to detect master repo boundary
