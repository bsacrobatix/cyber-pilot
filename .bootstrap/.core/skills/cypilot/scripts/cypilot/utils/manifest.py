...
def _require_list(
    raw: Dict[str, Any], field: str, path: Path, section: str, idx: int, cid: str,
) -> List[Any]:
    """Return *field* from *raw* as a list, raising ValueError if it isn't one."""
    val = raw.get(field, [])
    if not isinstance(val, list):
        raise ValueError(
            f"{path}: [[{section}]][{idx}] ('{cid}'): "
            f"'{field}' must be an array, got {type(val).__name__}"
        )
    return list(val)
...
def _parse_v2_sections(path: Path, data: Dict[str, Any]) -> ManifestV2:
    ...
    if not isinstance(inc, list):
        raise ValueError(f"{path}: 'includes' must be an array, got {type(inc).__name__}")
    includes = list(inc)
...
def _parse_base_fields(raw: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(raw_agents, list):
        raise ValueError(f"Component '{comp_id}': 'agents' must be an array, got {type(raw_agents).__name__}")
    return {
        "id": comp_id,
    ...
    for idx, raw in enumerate(raw_agents):
        ...
        if not isinstance(raw_tools, list):
            raise ValueError(
                ...
                f"'tools' must be an array, got {type(raw_tools).__name__}"
            )
        raw_disallowed = raw.get("disallowed_tools", [])
        if not isinstance(raw_disallowed, list):
            raise ValueError(
                f"{path}: [[agents]][{idx}] ('{base['id']}'): "
                f"'disallowed_tools' must be an array, got {type(raw_disallowed).__name__}"
            )
        tools = list(raw_tools)
        disallowed_tools = list(raw_disallowed)
        ...
        agents.append(AgentEntry(
            **base,
            ...
            skills=_require_list(raw, "skills", path, "agents", idx, base["id"]),
...
def resolve_includes(
    manifest: ManifestV2,
    manifest_dir: Path,
    include_chain: Optional[Tuple[Path, ...]] = None,
    ...
    Args:
        ...
        include_chain: Tuple of already-visited absolute manifest file paths
                       in traversal order (used for circular detection and
                       error messages). Pass ``None`` for the initial call.
    ...
    for include_path_str in manifest.includes:
        ...
        if resolved in include_chain:
            chain_str = " -> ".join(str(p) for p in include_chain) + f" -> {resolved}"
            raise ValueError(
        ...
        new_chain: Tuple[Path, ...] = include_chain + (resolved,)
    ...
    for idx, res in enumerate(resources):
        ...
        if not isinstance(rid, str) or not rid.strip():
            ...
            errors.append(
                ...
                "(lowercase letter start, then lowercase alphanumeric, underscore, or hyphen)"
            )
