...
Covers:
  ...
  - _render_toml_agents: multi-line developer_instructions round-trip
...
# ===========================================================================
# _render_toml_agents (legacy subagent TOML)
# ===========================================================================

class TestRenderTomlAgentsInjection(unittest.TestCase):
    """TOML injection tests for the legacy _render_toml_agents path."""

    def test_backslash_in_agent_path_roundtrips(self):
        """Agent paths with backslashes produce valid TOML."""
        agents = [_make_semantic_agent("t")]
        paths = {"t": "path\\to\\agent.md"}
        result = _render_toml_agents(agents, paths)
        data = _roundtrip_toml(result)
        instructions = data["agents"]["t"]["developer_instructions"]
        self.assertIn("path\\to\\agent.md", instructions)

    def test_triple_quote_in_agent_path_roundtrips(self):
        agents = [_make_semantic_agent("t")]
        paths = {"t": 'before"""after.md'}
        result = _render_toml_agents(agents, paths)
        data = _roundtrip_toml(result)
        self.assertIn('before"""after.md', data["agents"]["t"]["developer_instructions"])

    def test_backslash_n_in_description_roundtrips(self):
        """Literal \\n in description must not become a newline."""
        agents = [_make_semantic_agent("t", description=r"use \n here")]
        paths = {"t": "@/t.md"}
        result = _render_toml_agents(agents, paths)
        data = _roundtrip_toml(result)
        self.assertEqual(data["agents"]["t"]["description"], r"use \n here")
